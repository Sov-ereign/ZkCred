 ...
 * Provides Google OAuth, Manual Auth (Register/Login), and MongoDB Audit Persistence.
 */
...
app.post("/api/auth/google", async (req, res) => {
  try {
    const { name, email, googleId, avatarUrl, walletAddress } = req.body;
    if (!email || !name) {
      return res.status(400).json({ error: "Google OAuth email and name required" });
    }
